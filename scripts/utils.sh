...
PEER0_PRODUCER_CA=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/peerOrganizations/producer.example.com/peers/peer0.producer.example.com/tls/ca.crt
PEER1_PRODUCER_CA=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/peerOrganizations/producer.example.com/peers/peer1.producer.example.com/tls/ca.crt
...
PEER0_DISTRIBUTOR_CA=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/peerOrganizations/distributor.example.com/peers/peer0.distributor.example.com/tls/ca.crt
PEER0_RETAILER_CA=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/peerOrganizations/retailer.example.com/peers/peer0.retailer.example.com/tls/ca.crt
PEER1_RETAILER_CA=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/peerOrganizations/retailer.example.com/peers/peer1.retailer.example.com/tls/ca.crt 
...
PEER1_CONSUMER_CA=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/peerOrganizations/consumer.example.com/peers/peer1.consumer.example.com/tls/ca.crt
...
PRODUCER_MSP=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/peerOrganizations/producer.example.com/users/Admin@producer.example.com/msp
...
DISTRIBUTOR_MSP=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/peerOrganizations/distributor.example.com/users/Admin@distributor.example.com/msp
RETAILER_MSP=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/peerOrganizations/retailer.example.com/users/Admin@retailer.example.com/msp
...
setGlobals() {
    ...
    if [ $ORG -eq 1 ]; then
        ...
        CORE_PEER_TLS_ROOTCERT_FILE=$PEER0_PRODUCER_CA
        CORE_PEER_MSPCONFIGPATH=$PRODUCER_MSP
        if [ $PEER -eq 0 ]; then
...
instantiateChaincode() {
    ...
    if [ -z $CORE_PEER_TLS_ENABLED -o $CORE_PEER_TLS_ENABLED = "false" ]; then
        set -x
        peer chaincode instantiate -o orderer.example.com:7050 -C $CHANNEL_NAME -n $CC_NAME -v 1.0 -c '{"Args":[""]}' -P "OR ('ProducerMSP.peer', 'ManufacturerMSP.peer','DistributorMSP.peer','RetailerMSP.peer','ConsumerMSP.peer')">&log.txt
        res=$?
        set +x
    else
        set -x
        peer chaincode instantiate -o orderer.example.com:7050 --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n $CC_NAME  -v 1.0 -c '{"Args":[""]}' -P "OR ('ProducerMSP.peer', 'ManufacturerMSP.peer','DistributorMSP.peer','RetailerMSP.peer','ConsumerMSP.peer')" >&log.txt
        res=$?
        set +x
    fi
    sleep $DELAY
