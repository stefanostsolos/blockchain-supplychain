...
function generateChannelArtifacts() {
    ...
    echo "######################################################################"
    set -x
    configtxgen -profile TraceOrgsChannel -outputAnchorPeersUpdate ./artifacts/network/ProducerMSPanchors.tx -channelID $CHANNEL_NAME -asOrg ProducerMSP
    res=$?
    set +x
    if [ $res -ne 0 ]; then
    ...
    echo "######################################################################"
    set -x
    configtxgen -profile TraceOrgsChannel -outputAnchorPeersUpdate ./artifacts/network/DistributorMSPanchors.tx -channelID $CHANNEL_NAME -asOrg DistributorMSP
    res=$?
    set +x
    if [ $res -ne 0 ]; then
    ...
    echo "######################################################################"
    set -x
    configtxgen -profile TraceOrgsChannel -outputAnchorPeersUpdate ./artifacts/network/RetailerMSPanchors.tx -channelID $CHANNEL_NAME -asOrg RetailerMSP
    res=$?
    set +x
    if [ $res -ne 0 ]; then
